...
APP_BASENAME="ODTemp Logger"
APP_FILENAME="${APP_BASENAME}.app"
DISPLAY_NAME="ODTemp Logger"
BUNDLE_ID="com.opendev.odtemp-logger"
...
DMG_FILENAME="ODTemp_Logger_${APP_VERSION}_macos12_universal.dmg"
...
for tool in go clang lipo otool sips iconutil plutil codesign hdiutil ditto shasum; do
...
BUILD_DIR="$(mktemp -d "${TMPDIR:-/tmp}/odtemp-logger-dmg.XXXXXX")"
...
AMD64_BIN="${BUILD_DIR}/odtemp-logger-amd64"
ARM64_BIN="${BUILD_DIR}/odtemp-logger-arm64"
UNIVERSAL_BIN="${BUILD_DIR}/odtemp-logger"
...
ICONSET="${BUILD_DIR}/ODTemp.iconset"
mkdir -p "$ICONSET"
...
iconutil -c icns "$ICONSET" -o "${RESOURCES_DIR}/ODTemp.icns"
...
HELP_OUTPUT="$("${MOUNTED_APP}/Contents/MacOS/${APP_BASENAME}" -h 2>&1)"
if [[ "$HELP_OUTPUT" != *"Версия: ${APP_VERSION}"* ]]; then
	echo "Ошибка: справка приложения не содержит версию ${APP_VERSION}" >&2
	exit 1
fi
detach_image
